...
\keywords{syntax, parsing, type systems, module systems, macro systems, hygiene, pattern matching, bidirectional type systems, metaprogramming}
...
Third-party library providers are, justifiably, envious of this special arrangement -- they would like to be able to define syntactic sugar for constructing and pattern matching on values of types that they have defined, in a decentralized manner. For example, 1) the provider of a ``collections'' library would like to define syntactic sugar for finite sets and dictionaries; 2) the provider of a ``web programming'' library would like syntactic sugar for HTML and JSON values; and 3) the provider of a ``chemistry'' library would like syntactic sugar for chemical structures based on the SMILES standard.

...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common to define ``library-specific syntactic sugar'', i.e. {derived forms} that decrease the cognitive cost of working with select library constructs. 
For example, the textual syntax of Standard ML defines derived forms for introducing and pattern matching on lists. Semantically, lists are defined in the SML Basis library (i.e. SML's ``standard library''.)

Defining derived forms for working with third-party library constructs, e.g. ordered finite maps, HTML elements or regular expressions, requires defining ``library-specific'' (a.k.a. ``domain-specific'') dialects of the textual syntax. Unfortunately, combining such syntax dialects, when possible at all, is not guaranteed to conserve \emph{determinism}, i.e. syntactic ambiguities can be introduced. Moreover, it becomes difficult for programmers to reason in a disciplined manner about types and binding given only the text of a program that uses many unfamiliar derived forms. In short, the dialect-oriented approach is difficult to reconcile with the best practices of  ``programming in the large.''
...
\begin{itemize}
\item a \emph{type discipline}, meaning that the programmer can reason about types without examining literal expansions directly; and 
\item a \emph{hygienic binding discipline}, meaning that 
  \begin{enumerate}
    \item literal expansions cannot (accidentally or intentionally) shadow bindings that appear at the application site; and 
  \item literal expansions cannot make any assumptions about the application site typing context. Instead, all interactions with the application site bindings are \emph{parametric}.
  \end{enumerate}
\end{itemize}
\noindent

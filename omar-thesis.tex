...
\keywords{concrete syntax, parsing, macro systems, hygiene, type systems, module systems, pattern matching, elaboration semantics, bidirectional typing, type inference, extensible languages, metaprogramming}
...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common to define derived forms that decrease the syntactic cost of working with certain library constructs. 
...
Defining derived forms for working with other library constructs, e.g. dictionaries, HTML elements or regular expressions, requires defining new ``library-specific'' (a.k.a. ``domain-specific'') dialects of the textual syntax (perhaps with the aid of a tool, e.g. a parser generator). Unfortunately, clients cannot always combine these  syntactic dialects, limiting the choices available to them. Additionally, it can become difficult for clients to reason in a disciplined manner about types and binding in a program that uses unfamiliar derived forms, because these forms can have arbitrary desugarings.
...
\begin{itemize}
\item a \emph{type discipline}, meaning that a programmer examining a well-typed term can determine its type without examining its expansion directly (by examining type annotations on the TSMs being applied); and 
...
\end{itemize}
\noindent
Put pithily, we define a programming language in the ML tradition with a \emph{reasonably programmable syntax}.

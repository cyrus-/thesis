...
numbers=left, numberstyle=\scriptsize\color{gray}\ttfamily, language=ML,moredelim=[il][\sffamily]{?},showspaces=false,showstringspaces=false,xleftmargin=15pt, morekeywords=[1]{tyfam,opfam,let,fn,val,def,casetype,objtype,metadata,of,*,datatype,new,toast,syntax,module,where,import,for,ana,syn,opcon,tycon,metasignature,metamodule,metasig,metamod,static,at,tycase,mod,macro,match,pattern,in,patterns,expressions,implicit,forall,rectype,fold,unfold,inj},deletekeywords={double},classoffset=0,belowskip=\smallskipamount,
...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common to define ``library-specific syntactic sugar'', i.e. \emph{derived forms} that decrease the cognitive cost of working with select library constructs.
% -- library-specific ``syntactic sugar''. 
For example, the textual syntax of Standard ML (like that of many languages) defines derived forms for introducing and pattern matching on lists. %Semantically, lists are defined (as a recursive datatype) in the SML Basis library a.

Defining derived forms for working with third-party library constructs, e.g. ordered finite maps, HTML elements or regular expressions, requires defining ``library-specific'' (a.k.a. ``domain-specific'') dialects of the textual syntax. Unfortunately, combining such syntax dialects, when possible at all, is not guaranteed to conserve \emph{determinism}, i.e. syntactic ambiguities can be introduced. Moreover, it becomes difficult for programmers to reason in a disciplined manner about types and binding given only the text of a program that uses unfamiliar derived forms. %In short, the dialect-oriented approach is unreasonable for programming ``in the large''.\todo{mention and cite this}

In this work, we introduce and formally define \emph{typed syntax macros (TSMs)}, which reduce the need for \emph{ad hoc} derived forms and library-specific syntax dialects by giving library providers the ability to programmatically control the parsing and expansion, at compile-time, of expressions and patterns of (generalized) literal form. Library clients can use any combination of TSMs in a program without needing to consider the possibility of syntactic conflicts between them, and the language validates each expansion that a TSM generates in order to maintain:
\begin{itemize}
\item a \emph{type discipline}, meaning that a programmer encountering a literal form can determine its type without examining its expansion directly; and 
\item a \emph{hygienic binding discipline}, meaning that 
  \begin{enumerate}
    \item the expansion cannot (accidentally or intentionally) shadow bindings that appear at the application site; and 
  \item the expansion cannot make any assumptions about the application site typing context. Instead, all interactions with the application site bindings are \emph{parametric}.
  \end{enumerate}
\end{itemize}
\noindent
Support for partial parameter application in TSM abbreviations reduces the syntactic cost of this explicit parameter-passing style.

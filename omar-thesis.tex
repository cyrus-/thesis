...
This work formally introduces \emph{typed literal macros (TLMs)}, which reduce the need for library-specific syntax dialects by giving library providers the ability to programmatically control the parsing and expansion, at ``compile-time'', of expressions and patterns of \emph{generalized literal form}. Library clients can use any combination of TLMs in a program without needing to consider the possibility of syntactic conflict,  because the context-free syntax of the language is never extended (rather, it is  contextually repurposed.) Moreover, the language validates each expansion that a TLM generates in order to maintain useful abstract reasoning principles. In particular, expansion validation allows the TLM client to maintain:
\begin{itemize}
...
\item a \emph{hygienic binding discipline}, meaning that the client can be sure that:
  \begin{enumerate}
    \item  literal expansions cannot shadow bindings that appear at the TLM application site; and 

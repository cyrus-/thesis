...
\begin{abstract}
\noindent
Full-scale functional programming languages often make \emph{ad hoc} choices in the design of their textual syntax. For example, nearly all major functional languages build in derived forms for lists, but introducing derived forms  for other library constructs, e.g. for HTML elements or regular expressions, requires forming new syntactic dialects of these languages. Programmers cannot  combine  syntactic dialects in general, limiting the choices ultimately available to them. In this work, we introduce and formally specify language primitives that mitigate the need for \emph{ad hoc} derived forms and syntactic dialects by giving library providers the ability to programmatically control the expansion of certain \emph{generalized literal forms} while maintaining a  type discipline, a binding discipline and compositional reasoning principles.
\end{abstract}

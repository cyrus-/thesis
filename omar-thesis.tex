...
numbers=left, numberstyle=\scriptsize\color{gray}\ttfamily, language=ML,moredelim=[il][\sffamily]{?},showspaces=false,showstringspaces=false,xleftmargin=15pt, morekeywords=[1]{tyfam,opfam,let,fn,val,def,casetype,objtype,metadata,of,*,datatype,new,toast,syntax,module,where,import,for,ana,syn,opcon,tycon,metasignature,metamodule,metasig,metamod,static,at,tycase,mod,macro,match,pattern,in,patterns,expressions,implicit,forall},deletekeywords={double},classoffset=0,belowskip=\smallskipamount,
...
\keywords{syntax, parsing, type systems, module systems, pattern matching, elaboration semantics, bidirectional typing, macro systems, hygiene, metaprogramming}
...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common to define derived forms that decrease the cognitive cost of working with certain library constructs (i.e. ``syntactic sugar''). 
% -- library-specific ``syntactic sugar''. 
For example, the textual syntax of Standard ML (like that of many languages) defines derived forms for introducing and pattern matching on lists, which are defined in the SML Basis library. %Semantically, lists are defined (as a recursive datatype) in the SML Basis library a.

Defining derived forms for working with third-party library constructs, e.g. order-preserving dictionaries, HTML elements or regular expressions, requires defining new ``library-specific'' (a.k.a. ``domain-specific'') dialects of the textual syntax (often with the aid of a tool, like a parser generator). Unfortunately, clients cannot always combine these syntax dialects, either because the dialects are specified using different formalisms, or because the combined syntax is no longer deterministic. Moreover, it can become difficult for clients to reason in a disciplined manner about types and binding in a program that uses unfamiliar derived forms, because these forms can have arbitrary desugarings.

In this work, we introduce and formally define \emph{typed syntax macros} (TSMs), which reduce the need for \emph{ad hoc} derived forms and library-specific syntax dialects by giving library providers the ability to programmatically control the parsing and expansion, at compile-time, of certain \emph{generalized literal forms}. Library clients can use any combination of TSMs in a program without needing to consider the possibility of conflicts between them, and the language validates each expansion that a TSM generates to maintain:
\begin{itemize}
\item a \emph{type discipline}, meaning that a programmer examining a well-typed term can determine its type without examining its expansion directly; and 
\item a \emph{hygienic binding discipline}, meaning that the expansion cannot make any assumptions about identifiers bound at the TSM application site, nor  introduce ``hidden bindings'' into subterms. Instead, all interactions with the surrounding context and with sub-terms are explicit, through parameters.
\end{itemize}
\noindent
Put pithily, we describe a programming language in the ML tradition with a \emph{reasonably} programmable syntax.

We discuss both explicit application of TSMs and implicit, type-directed application of TSMs, which further reduces cognitive cost (particularly for parameterized TSMs).
\end{abstract}

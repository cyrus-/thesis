...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common practice to define derived forms for  standard library constructs.   
%, i.e. {derived forms} that decrease the cognitive cost of idioms involving select library constructs. 
For example, the textual syntax of Standard ML (SML) defines derived forms for constructing and pattern matching on lists. %This decreases the cognitive cost of working with lists. %Semantically, lists are defined in the SML Basis library (i.e. SML's ``standard library''.)

Third-party library providers are, justifiably, envious of this arrangement. The problem is that while it is straightforward to define other useful derived forms (e.g. for maps, HTML elements or regular expressions) in separately developed ``domain-specific'' dialects of the textual syntax, combining these syntax dialects, when possible at all, is not guaranteed to conserve syntactic {determinism} (i.e. syntactic conflicts can arise.) Moreover, it can become difficult for programmers to reason in a disciplined manner about types and binding when examining the text of a program that uses unfamiliar derived forms. In short, the dialect-oriented approach is difficult to reconcile with the best practices of  ``programming in the large.''

In this work, we introduce and formally define \emph{typed syntax macros (TSMs)}, which reduce the need for \emph{ad hoc} derived forms and library-specific syntax dialects by giving library providers the ability to programmatically control the parsing and expansion, at compile-time, of expressions and patterns of (generalized) literal form. Library clients can use any combination of TSMs in a program without needing to consider the possibility of syntactic conflict between them because the syntax of the language is never extended (only contextually repurposed.) The language validates each expansion that a TSM generates in order to maintain:
\begin{itemize}
\item a \emph{type discipline}, meaning that the programmer can reason about types without examining literal expansions; and 
\item a \emph{hygienic binding discipline}, meaning that 
  ...
  \item literal expansions cannot refer to application site bindings directly. Instead, all interactions with  application site bindings go through \emph{spliced subterms} and \emph{parameters}. Partial parameter application helps reduce the syntactic cost of this explicit parameter passing style.
  \end{enumerate}
\end{itemize}
\noindent

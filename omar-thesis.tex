...
\begin{abstract}
\noindent
When designing a human-facing textual syntax for a programming language, it is common to define ``library-specific syntactic sugar''. 
...
Defining derived forms for working with third-party library constructs, e.g. ordered finite maps, HTML elements or regular expressions, requires defining ``library-specific'' (a.k.a. ``domain-specific'') dialects of the textual syntax. Unfortunately, combining such syntax dialects, when possible at all, is not guaranteed to conserve \emph{determinism}, i.e. syntactic ambiguities can be introduced. Moreover, it can become difficult for programmers to reason in a disciplined manner about types and binding given the text of a program that uses many unfamiliar derived forms. In short, the dialect-oriented approach is difficult to reconcile with the best practices of  ``programming in the large.''

In this work, we introduce and formally define \emph{typed syntax macros (TSMs)}, which reduce the need for \emph{ad hoc} derived forms and library-specific syntax dialects by giving library providers the ability to programmatically control the parsing and expansion, at compile-time, of expressions and patterns of (generalized) literal form. Library clients can use any combination of TSMs in a program without needing to consider the possibility of syntactic conflict between them, and the language validates each expansion that a TSM generates in order to maintain:
\begin{itemize}
\item a \emph{type discipline}, meaning that the programmer can reason about types while holding literal expansions abstract; and 
\item a \emph{hygienic binding discipline}, meaning that 
  ...
  \item literal expansions cannot interact with application site bindings directly. Instead, all interactions with the  application site bindings are \emph{parametric}. Support for partial parameter application reduces the syntactic cost of this explicit parameter passing style.
  \end{enumerate}
\end{itemize}
\noindent

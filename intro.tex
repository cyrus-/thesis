...
Experienced mathematicians and programmers define formal structures \emph{compositionally}, drawing from libraries of other, often more general, formal structures. The benefits of this approach are clear: composing general-purpose structures saves time and increases reliability.  The problem motivating this work is that abstraction and composition can also incur a non-trivial syntactic cost. %This can lower productivity, readability and other quality attributes of interest.  %This saves time, one does not need to establish associated definitions and proofs anew, for each specialized structure of interest.

Consider, for example, natural numbers. It is straightforward to define the natural numbers as inductive structures -- in a document like this, we can do so with the following inductive grammar:
...
The problem with this approach is once again that drawing a non-trivial $\simplelang$ expressions in operational form is syntactically costly, e.g. consider the following example:
...
Consequently, the problems of syntactic cost just discussed at the level of the ambient mathematics also arise ``one level down'', i.e. when writing programs. For example, we want syntactic sugar not only for mathematical natural numbers, lists and $\simplelang$ expressions, but also for \emph{encodings} of these structures within a general-purpose language.% .) %https://github.com/jonsterling/sml-abt or https://github.com/RedPRL/sml-typed-abts.) 
...
Constructing a formal syntax definition is not itself an unusually difficult task for an experienced programmer, and there are many \emph{syntax definition systems} that help with this task (Sec. \ref{sec:existing-approaches} will review some of these.) The problem is that when designing a syntax for a general-purpose language, the language designer cannot hope to anticipate all library constructs for which derived forms might one day be useful. At best, the language designer can bundle certain libraries together into a ``standard library'', and privilege select constructs defined in this library with derived forms. 
...
This suggests a more parsimonious approach: to eliminate derived forms  specific to standard library constructs from the language definition in favor of mechanisms that give more syntactic control to third-party library providers.

We will return to give a detailed overview of various existing mechanisms of syntactic control in Section \ref{sec:existing-approaches}. In the remainder of this section, we will give a more concise overview of existing mechanisms and speak generally about the problems that these mechanisms present, to motivate our novel contributions in this area.

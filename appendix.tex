...
\ificfp
\chapter{\texorpdfstring{$\miniVersePat$ (and $\miniVerseUE$)}{miniVerseS (and miniVerseSE)}}
\else
...
\ificfp
This section defines $\miniVersePat$, the calculus of simple expression and pattern TSMs. For some readers, it might be useful to snip out pattern matching to get a language strictly of expression TSMs. To support that, one can omit the segments typeset in gray backgrounds below to recover $\miniVerseUE$, a calculus of simple expression TSMs. We have included the necessary eliminators below (they are technically redundant with pattern matching, but don't hurt things so they're left in white.)
\else
...
If $\cvalidT{\Delta}{\tsceneU{\uDD{\uD}{\Delta_\text{app}}}{b}}{\ctau}{\tau}$ where $\summaryOf{\ctau} = \sseq{\acesplicedt{m_i}{n_i}}{n}$ then all of the following hold:
\begin{enumerate}
...
\begin{byCases}
  \item[\text{(\ref{rule:cvalidT-U-tvar})}] 
    ...
    \item \summaryOf{\ctau} = \emptyset \BY{definition}
...
\subsubsection{TSM Type and Expression Expansion}
...
\subsubsection{Statics of the TSM Language}
...
\subsubsection{Dynamics of the TSM Language}
...
\noindent\fbox{$\strut\tsmexpStepsPat{\Omega}{\Psi}{\epsilon}{\epsilon'}$}~~ppTSM expression $\epsilon$ transitions to $\epsilon'$
...
\noindent\fbox{$\strut\tsmexpMultistepsPat{\Omega}{\Psi}{\epsilon}{\epsilon'}$}~~ppTSM expression $\epsilon$ transitions in multiple steps to $\epsilon'$
...
\noindent\fbox{$\strut\tsmexpEvalsPat{\Omega}{\Psi}{\epsilon}{\epsilon'}$}~~ppTSM expression $\epsilon$ normalizes to $\epsilon'$
...
\begin{lemma}[ppTSM Regularity]
\label{lemma:ppTSM-regularity}
...
\begin{lemma}[peTSM Unicity]
\label{lemma:peTSM-unicity}
...
\begin{lemma}[ppTSM Unicity]
\label{lemma:ppTSM-unicity}
...
\begin{corollary}[peTSM Preservation (Multistep)]
...
\begin{corollary}[peTSM Preservation (Evaluation)]
...
\begin{corollary}[ppTSM Preservation (Multistep)]
...
\begin{corollary}[ppTSM Preservation (Evaluation)]
...
\begin{enumerate}
  ...
  \item If $\cvalidK{\Omega}{\csceneP{\omega : \OParams}{\uOmega}{b}}{\cekappa}{\kappa}$
    where $\summaryOf{\cekappa} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon}$ then 
    \begin{enumerate}
      \item $\sseq{\kExpands{\uOmega}{\parseUKindF{\bsubseq{b}{m_i}{n_i}}}{\kappa_i}}{\nkind}$
      \item $\sseq{\cvalidK{\OParams}{\csceneP{\omega : \OParams}{\uOmega}{b}}{\cekappa'_i}{\kappa'_i}}{\ncon}$
      \item $\sseq{\cExpands{\uOmega}{\parseUConF{\bsubseq{b}{m'_i}{n'_i}}}{c_i}{[\omega]\kappa'_i}}{\ncon}$
...
\begin{proof} % TODO this one is sloppy
  ...
  \item $\summaryOf{\acesplicedc{m}{n}{\cekappa}} = \summaryOf{\cekappa} \cup \{ \acesplicedc{m}{n}{\cekappa} \}$ \BY{definition} \pflabel{summaryOf1}
  \item $\summaryOf{\cekappa} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon - 1}$ \BY{definition} \pflabel{summaryOf2}
...
\begin{enumerate}
  \item If $\cvalidEP{\Omega}{\esceneP{\omega : \OParams}{\uOmega}{\uPsi}{\uPhi}{b}}{\ce}{e}{\tau_\text{proto}}$
    where $\summaryOf{\ce} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon} \cup \sseq{\acesplicede{m''_i}{n''_i}{\ctau_i}}{\nexp}$ then 
    \begin{enumerate}
      \item $\sseq{\kExpands{\uOmega}{\parseUKindF{\bsubseq{b}{m_i}{n_i}}}{\kappa_i}}{\nkind}$
      \item $\sseq{\cvalidK{\OParams}{\csceneP{\omega : \OParams}{\uOmega}{b}}{\cekappa'_i}{\kappa'_i}}{\ncon}$
      \item $\sseq{\cExpands{\uOmega}{\parseUConF{\bsubseq{b}{m'_i}{n'_i}}}{c_i}{[\omega]\kappa'_i}}{\ncon}$
  ...
  \item If $\cvalidRP{\Omega}{\esceneP{\omega : \OParams}{\uOmega}{\uPsi}{\uPhi}{b}}{\crv}{r}{\tau}{\tau'}$
    where $\summaryOf{\crv} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon} \cup \sseq{\acesplicede{m''_i}{n''_i}{\ctau_i}}{\nexp}$ then 
    \begin{enumerate}
      \item $\sseq{\kExpands{\uOmega}{\parseUKindF{\bsubseq{b}{m_i}{n_i}}}{\kappa_i}}{\nkind}$
      \item $\sseq{\cvalidK{\OParams}{\csceneP{\omega : \OParams}{\uOmega}{b}}{\cekappa'_i}{\kappa'_i}}{\ncon}$
      \item $\sseq{\cExpands{\uOmega}{\parseUConF{\bsubseq{b}{m'_i}{n'_i}}}{c_i}{[\omega]\kappa'_i}}{\ncon}$
  ...
  \begin{enumerate}
    \item \begin{byCases}
      ...
      \begin{pfsteps*}
        ...
        \item $\uOmega=\uOmegaEx{\uD}{\uG}{\uMctx}{\Omega_\text{app}}$ \BY{assumption}
        \item $\domof{\Omega} \cap \domof{\Omega_\text{app}} = \emptyset$ \BY{assumption}
        \item $\summaryOf{\ce} = \summaryOf{\ctau} \cup \{ \acesplicede{m}{n}{\ctau} \}$ \BY{assumption}
        \item $\summaryOf{\ctau} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon}$  \BY{definition} \pflabel{sumC}
    ...
    \begin{byCases}
      \item[\text{(\ref{rule:cvalidR-P})}] ~
        \begin{pfsteps*}
          ...
          \item $\summaryOf{\crv} = \summaryOf{e}$ \BY{definition} \pflabel{summ}
...
\begin{byCases}
  \item[\text{(\ref{rule:expandsP-subsume})}] ~
    \begin{pfsteps*}
      ...
      \item $\segOK{\segof{\ce}}{b}$ \BY{IH on \pfref{expands}} \pflabel{K}
  ...
  \item[\text{(\ref{rule:expandsP-apuetsm})}] ~
    \begin{pfsteps*}
      ...
      \item $\hastsmtypeExp{\Omega_\text{app}}{\Psi}{\epsilon}{\aetype{\tau_\text{final}}}$ \BY{Theorem \ref{thm:peTSM-expression-expansion} on \pfref{tsmexpExpands}} \pflabel{hastsmtype}
      \item $\hastsmtypeExp{\Omega_\text{app}}{\Psi}{\epsilon_\text{normal}}{\aetype{\tau_\text{final}}}$ \BY{Corollary \ref{thm:peTSM-preservation-evaluation} on \pfref{hastsmtype} and \pfref{tsmexpEvals}} \pflabel{hastsmtypeNormal}
      ...
      \item $\aetype{\tau_\text{final}} = [\omega]\aetype{\tau_\text{proto}}$ \BY{Theorem \ref{lemma:peTSM-unicity} on \pfref{hastsmtypeNormal} and \pfref{hastsmtypeNormal2}} \pflabel{tsmtypeseq}
      \item $\tau_\text{final}=[\omega]\tau_\text{proto}=\tau$ \BY{definition of substitution and \pfref{tsmtypeseq} and \pfref{tau}} \pflabel{feqp}
      \item $\istsmty{\Omega_\text{app}}{\aetype{\tau_\text{final}}}$ \BY{Lemma \ref{lemma:peTSM-regularity} on \pfref{hastsmtype}} \pflabel{istsmty}
...
If $\cvalidPP{\uOmega'}{\psceneP{\omega : \Omega_\text{params}}{\uOmega}{\uPhi}{b}}{\cpv}{p}{\tau}$ where \[
  \begin{array}{lll}
  \summaryOf{\cpv} & = &\sseq{\acesplicedk{m_i}{n_i}}{\nkind} \\
...
\begin{byCases}
  ...
  \item[\text{(\ref{rule:cvalidPP-spliced})}] ~
    \begin{pfsteps*}
      ...
      \item $\summaryOf{\cpv} = \summaryOf{\ctau} \cup \{ \acesplicedp{m}{n}{\ctau} \}$ \BY{definition} \pflabel{summ}
...
\begin{byCases}
  \item[\text{(\ref{rule:patExpandsP-subsume})}] ~
    \begin{pfsteps*}
      ...
      \item $\segOK{\segof{\cpv}}{b}$\BY{IH on \pfref{patExpands}} \pflabel{K}
  ...
  \item[\text{(\ref{rule:patExpandsP-apuptsm})}] ~
    \begin{pfsteps*}
      ...
      \item $\cvalidPP{\uOmega'}{\psceneP{\omega : \Omega_\text{params}}{\uOmega}{\uPhi}{b}}{\cpv}{p}{\tau_\text{proto}}$ \BY{assumption} \pflabel{cvalidPP}
        \item $\tau=[\omega]\tau_\text{proto}$ \BY{assumption} \pflabel{tau}
      \item $\hastsmtypePat{\Omega_\text{app}}{\Psi}{\epsilon}{\aetype{\tau_\text{final}}}$ \BY{Theorem \ref{thm:ppTSM-expression-expansion} on \pfref{tsmexpExpands}} \pflabel{hastsmtype}
      \item $\hastsmtypePat{\Omega_\text{app}}{\Psi}{\epsilon_\text{normal}}{\aetype{\tau_\text{final}}}$ \BY{Corollary \ref{thm:ppTSM-preservation-evaluation} on \pfref{hastsmtype} and \pfref{tsmexpEvals}} \pflabel{hastsmtypeNormal}
      ...
      \item $\aetype{\tau_\text{final}} = [\omega]\aetype{\tau_\text{proto}}$ \BY{Theorem \ref{lemma:ppTSM-unicity} on \pfref{hastsmtypeNormal} and \pfref{hastsmtypeNormal2}} \pflabel{tsmtypeseq}
      \item $\tau_\text{final}=[\omega]\tau_\text{proto}=\tau$ \BY{definition of substitution and \pfref{tsmtypeseq} and \pfref{tau}} \pflabel{feqp}
      \item $\istsmty{\Omega_\text{app}}{\aetype{\tau_\text{final}}}$ \BY{Lemma \ref{lemma:ppTSM-regularity} on \pfref{hastsmtype}} \pflabel{istsmty}
      \item $\haskind{\Omega_\text{app}}{\tau}{\akty}$ \BY{Inversion of Rule (\ref{rule:istsmty-type}) on \pfref{istsmty}} \pflabel{istype}
      \item $\cequal{\Omega_\text{app}}{\tau}{\tau}{\akty}$ \BY{Rule (\ref{rule:cequal-refl}) on \pfref{istype}} \pflabel{refl}
      \item $\issubtypeP{\Omega_\text{app}}{\tau}{\tau}$ \BY{Rule (\ref{rule:issubtypeP-equal}) on \pfref{refl}} \pflabel{subty}
      ...
      \item $\summaryOf{\cpv} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon} \cup
             \sseq{\acesplicedp{m''_i}{n''_i}{\ctau_i}}{\npat}
...
\begin{definition}[seTSM Definition Context Formation]\label{def:seTSM-def-ctx-formation-B} $\uetsmenv{\Delta}{\Psi}$ iff for each $\xuetsmbnd{a}{\tau}{\eparse} \in \Psi$, we have $\istypeU{\Delta}{\tau}$ and $\hastypeU{\emptyset}{\emptyset}{\eparse}{\aparr{\tBody}{\tParseResultExp}}$.\end{definition}

An \emph{spTSM definition context}, $\Phi$, is a finite function mapping each TSM name $a \in \domof{\Phi}$ to an \emph{expanded seTSM definition}, $\xuptsmbnd{a}{\tau}{\eparse}$, where $\tau$ is the spTSM's type annotation, and $\eparse$ is its parse function. We write $\Phi, \xuptsmbnd{a}{\tau}{\eparse}$ when $a \notin \domof{\Phi}$ for the extension of $\Phi$ that maps $a$ to $\xuptsmbnd{a}{\tau}{\eparse}$. We write $\uptsmenv{\Delta}{\Phi}$  when all the type annotations in $\Phi$ are well-formed assuming $\Delta$, and the parse functions in $\Phi$ are closed and of the appropriate type.

\begin{definition}[spTSM Definition Context Formation]\label{def:spTSM-def-ctx-formation-B} $\uptsmenv{\Delta}{\Phi}$ iff for each $\xuptsmbnd{a}{\tau}{\eparse} \in \Phi$, we have $\istypeU{\Delta}{\tau}$ and $\hastypeU{\emptyset}{\emptyset}{\eparse}{\aparr{\tBody}{\tParseResultPat}}$.\end{definition}
...
\item If ($\cana
  ...
  \begin{enumerate}
    ...
    %   {e'}{\tau}$
  \end{enumerate}
\item If $\cvalidR{\Delta}{\Gamma}{\esceneUP{\uDD{\uD}{\Delta_\text{app}}}{\uGG{\uG}{\Gamma_\text{app}}}{\uPsi}{\uPhi}{b}}{\crv}{r}{\tau}{\tau'}$ where \[\summaryOf{\crv} = \sseq{\acesplicedt{m'_i}{n'_i}}{\nty} \cup \sseq{\acesplicede{m_i}{n_i}{\ctau_i}}{\nexp}\] then all of the following hold:
  \begin{enumerate}
    \item $\sseq{
          \expandsTU{\uDD{\uD}{\Delta_\text{app}}}
          {
...
\begin{theorem}[seTSM Abstract Reasoning Principles - Explicit Application]
...
\end{enumerate}
\end{theorem}
\begin{proof} By rule induction over Rules (\ref{rules:eana-S}). The proof follows the proof of Theorem \ref{thm:tsc-SES}, differing only in how the TSM definition is looked up.
\end{proof}
...
\begin{theorem}[spTSM Abstract Reasoning Principles - Explicit Application]
...
\end{enumerate}
\end{theorem}
\begin{proof} By rule induction over Rules (\ref{rules:patExpands-B}). The proof follows the proof of Theorem \ref{thm:spTSM-Typing-Segmentation}, differing only in how the parse function is looked up.
\end{proof}

...
\item \fvof{p} = \domof{\Gamma} = \emptyset \BY{definition}
\end{pfsteps}
\resetpfcounter
...
\begin{pfsteps*}
  ...
  \item $\{\fvof{p_i} = \domof{\pctx_i}\}_{i \in \labelset}$ \BY{IH over \pfref{patType} and \pfref{istypeU-each}} \pflabel{biggy2}
  ...
  \item $\fvof{p} = \domof{\Gamma} = \emptyset$ \BY{definition and \pfref{biggy2}}
\end{pfsteps*}
\resetpfcounter
...
\begin{pfsteps*}
  ...
  \item $\fvof{p'} = \domof{\pctx}$
  \BY{IH on \pfref{patType} and \pfref{istypeTwo}} \pflabel{fv1}
  \item $\fvof{p} = \domof{\pctx}$ \BY{definition and \pfref{fv1}}
\end{pfsteps*}
\resetpfcounter
\end{byCases}
\end{proof}
...
\begin{byCases}
  \item[\text{(\ref{rule:mExpandsP-subsumes})}] ~
    ...
    \end{pfsteps*}
    \resetpfcounter
  \item[\text{(\ref{rule:mExpandsP-var}) \textbf{through} (\ref{rule:mExpandsP-mlet})}] In each of these cases, we apply the IH over each premise then apply the corresponding signature matching rule in Rules (\ref{rules:hassig}) and weakening as needed.
...
\begin{proof} 
  ...
  \begin{enumerate}
    \item ~
      \begin{byCases}
        \item[...] \todo{base and inductive cases}
        \item[...] \todo{spliced case}
          \begin{pfsteps*}
            \item \todo{steps}
          \end{pfsteps*}
          The conclusions hold as follows:
...
\begin{enumerate}
  ...
  \item If $\cvalidRP{\Omega}{\esceneP{\omega : \OParams}{\uOmega}{\uPsi}{\uPhi}{b}}{\crv}{r}{\tau}{\tau'}$
    and $\summaryOf{\crv} = \sseq{\acesplicedk{m_i}{n_i}}{\nkind} \cup \sseq{\acesplicedc{m'_i}{n'_i}{\cekappa'_i}}{\ncon} \cup \sseq{\acesplicede{m''_i}{n''_i}{\ctau_i}}{\nexp}$ then 
    \begin{enumerate}
      \item $\sseq{\kExpands{\uOmega}{\parseUKindF{\bsubseq{b}{m_i}{n_i}}}{\kappa_i}}{\nkind}$
      \item $\sseq{\cvalidK{\OParams}{\csceneP{\omega : \OParams}{\uOmega}{b}}{\cekappa'_i}{\kappa'_i}}{\ncon}$
      \item $\sseq{\cExpands{\uOmega}{\parseUConF{\bsubseq{b}{m'_i}{n'_i}}}{c_i}{[\omega]\kappa'_i}}{\ncon}$
...
\end{enumerate}
\end{theorem}
\begin{proof} By rule induction over Rules (\ref{rules:expandsP}).
\begin{byCases}
  \item[\text{(\ref{rule:expandsP-subsume})}] \todo{mod statement of theorem to be up to subtyping}
  \item[\text{(\ref{rule:expandsP-var}) \textbf{through} (\ref{rule:expandsP-mval})}] These cases do not syntactically apply.
  \item[\text{(\ref{rule:expandsP-apuetsm})}] ~
    \begin{pfsteps*}
      \item $\ue=\utsmap{\uepsilon}{b}$ \BY{assumption} \pflabel{ue}
    ...
    \begin{enumerate}
     \item \pfref{uomega}
     \item \pfref{upsi}
     \item \pfref{tsmexpExpands} and \pfref{feqp} and \pfref{hastype}
...
\end{theorem}
\begin{proof} 
By rule induction over Rules (\ref{rules:patExpandsP}).
\begin{byCases}
...
\end{theorem}
\begin{proof}
By mutual rule induction over Rules (\ref{rules:esyn-S}), Rules (\ref{rules:eana-S}), Rule (\ref{rule:rana-S}), Rules (\ref{rules:csyn}), Rules (\ref{rules:cana}) and Rule (\ref{rule:crana}). \todo{cases?}
...
\end{enumerate}
\end{theorem}
\begin{proof} By rule induction over Rules (\ref{rules:eana-S}). The proof follows the proof of Theorem \ref{thm:tsc-SES}. \todo{new case}
\end{proof}
...
\end{enumerate}
\end{theorem}
\begin{proof} By rule induction over Rules (\ref{rules:patExpands-B}). The proof follows the proof of Theorem \ref{thm:spTSM-Typing-Segmentation}. \todo{new case}
\end{proof}

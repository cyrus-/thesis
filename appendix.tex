...
\item We induct on the premise. In the following, let $\uOmega = \uOmegaEx{\uD}{\uG}{\uMctx}{\Omega_\text{app}}$ and $\uOmega' = \uOmegaEx{\uD'}{\uG'}{\uMctx'}{\Omega'}$.
  \begin{byCases}
    \item[\text{(\ref{rule:cvalidPP-wild}) through (\ref{rule:cvalidPP-in})}] These cases follow by applying the IH, part 2 and then applying the corresponding pattern rule in Rules (\ref{rules:patTypeP}).

    ...
    \item In the following, let $\escenev=\esceneP{\omega : \OParams}{\uOmegaEx{\uD}{\uG}{\uMctx}{\Omega_\text{app}}}{\uPsi}{\uPhi}{b}$.
     \begin{byCases}
      \item[\text{(\ref{rule:cvalidE-P-subsume})}] ~
        \begin{pfsteps*}
          \item $\cvalidEP{\Omega_\text{params}}{\escenev}{\ce}{e}{\tau'}$ \BY{assumption} \pflabel{cvalid}
          \item $\issubtypeP{\OParams}{\tau'}{\tau}$ \BY{assumption} \pflabel{issubtype}
          \item $\hastypeP{\Omega_\text{params} \cup \Omega_\text{app}}{e}{\tau'}$ \BY{IH, part 2(a) on \pfref{cvalid}} \pflabel{hastype}
          \item $\hastypeP{\Omega_\text{params} \cup \Omega_\text{app}}{e}{\tau}$ \BY{Rule (\ref{rule:hastypeP-subsume}) on \pfref{hastype} and \pfref{issubtype}}
        \end{pfsteps*}
        \resetpfcounter
      ...
      \item[\text{(\ref{rule:cvalidE-P-splicede})}] ~
        \begin{pfsteps*}
          ...
          \item $\hastypeP{\Omega_\text{app} \cup \OParams}{e}{[\omega]\tau'}$ \BY{Weakening on \pfref{hastype}}
        \end{pfsteps*}
        \resetpfcounter
    \end{byCases}
    \item \begin{byCases}
      ...
      \begin{pfsteps*}
        \item $\crv=\acematchrule{p}{\ce}$ \BY{assumption}
        \item $r=\aematchrule{p}{e}$ \BY{assumption}
        \item $\patTypePC{\OParams}{\Omega'}{p}{\tau}$ \BY{assumption} \pflabel{pattype}
        \item $\cvalidEP{\Gcons{\OParams}{\Omega'}}{\esceneP{\omega : \OParams}{\uOmegaEx{\uD}{\uG}{\uMctx}{\Omega_\text{app}}}{\uPsi}{\uPhi}{b}}{\ce}{e}{\tau'}$ \BY{assumption} \pflabel{cvalidEP}
        \item $(\OParams \cup \Omega') \cap \Omega_\text{app} = \emptyset$ \BY{identification convention} \pflabel{disjoint}
        \item $\hastypeP{\OParams \cup \Omega' \cup \Omega_\text{app}}{e}{\tau'}$ \BY{IH, part 2(a) on \pfref{cvalidEP} and \pfref{disjoint}} \pflabel{hastype}
        \item $\patTypePC{\OParams \cup \Omega_\text{app}}{\Omega'}{p}{\tau}$ \BY{Weakening on \pfref{pattype}} \pflabel{pattype2}
        \item $\ruleTypeP{\OParams \cup \Omega_\text{app}}{r}{\tau}{\tau'}$ \BY{Rule (\ref{rule:ruleTypeP}) on \pfref{pattype2} and \pfref{hastype}}
      \end{pfsteps*}
      \resetpfcounter
    \end{byCases}
  \end{enumerate}
...
\end{enumerate}
\end{theorem}
\begin{proof} \todo{lemma + proof} \end{proof}

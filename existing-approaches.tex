...
There are many existing approaches that a library provider seeking to decrease the cognitive cost of idioms involving definitions like those in the previous section can consider. 
...
We also defined a utility functor for regexes, \li{RXUtil}, in Figure \ref{fig:RXUtil}. As more idioms involving regexes arise, we can capture them by adding additional definitions to this functor (or some other such functor, if we cannot modify \li{RXUtil} itself.) For example, we can define a value that matches single digits in \li{RXUtil} as follows:
\begin{lstlisting}[numbers=none]
val digit = R.Or(R.Str "SSTR0ESTR", R.Or(R.Str "SSTR1ESTR", ...))
\end{lstlisting}
Similarly, we can define a function \li{repeat : R.t -> int -> R.t} that constructs a regex by sequentially repeating the given regex a given number of times (not shown.) 
Using these definitions, a client can define a regex that matches U.S. social security numbers as follows:
\begin{lstlisting}[numbers=none]
...
One limitation of this approach is that it provides no way to capture idioms at the level of patterns. Variables stand only for values.
...
Similarly, consider the textual syntax for regexes defined in the POSIX standard \cite{STD95954} (data suggests that most professional programmers are familiar with these forms \cite{Omar:2012:ACC:2337223.2337324}.) Under this syntax, the regex that matches DNA bases is drawn:
\begin{lstlisting}[numbers=none]
A|T|G|C
\end{lstlisting}
...
\d{3}-\d{2}-\d{4}
\end{lstlisting}
These drawings have substantially lower syntactic cost than the corresponding VerseML drawings shown earlier. Programmers familiar with the POSIX standard regex syntax would likely agree that these drawings have lower cognitive cost as well. 

\vspace{-6px}
\subsubsection{Dynamic String Parsing}\label{sec:dynamic-string-parsing}
We might attempt to approximate the POSIX standard regex syntax by defining a function \li{parse : string -> R.t} in \li{RXUtil} that parses a VerseML string representation of a POSIX regex form  to produce a regular expression value, or raise an exception if the input is malformed with respect to the POSIX specification. 

For example, a client can construct the regex matching DNA bases by applying \li{RU.parse} as follows:
\begin{lstlisting}[numbers=none]
...
\end{lstlisting}
because \verb|\d| is not a valid string escape character. In a small lab study, we observed that even experienced programmers made this mistake, and could not quickly diagnose the problem, if they had not used a regex library recently  \cite{Omar:2012:ACC:2337223.2337324}.
...
\end{lstlisting}
\item The next problem is that dynamic string parsing does not help lower the syntactic cost of compositional regex constructions. 

For example, the function \li{lookup_rx} in Figure \ref{fig:lookup_rx} constructs a regex from a string and another regex. We cannot use \li{RU.parse} to redraw this function equivalently, but at lower syntactic cost. 
...
We will describe derived forms that do capture the idioms of compositional regex construction in Sec. \ref{sec:syntax-dialects} (in particular, compare Figure \ref{fig:lookup_rx} with Figure \ref{fig:derived-spliced-subexpressions}.)
...
Both \lstinline{lookup_rx} and \lstinline{lookup_rx_insecure} have the same type, \li{string -> R.t}, and behave identically at many inputs, particularly the ``typical'' inputs (i.e. alphabetic names.) It is only when \li{lookup_rx_insecure} is applied to a string that parses as a regex that describes a regular language that also contains \emph{other} strings (whew) that it behaves incorrectly. 
...
This problem is, fundamentally, attributable to the programmer making a mistake in a misguided effort to decrease syntactic cost. However, the availability of a better approach for reducing syntactic cost would serve to make this class of mistakes less prevalent \cite{Bravenboer:2007:PIA:1289971.1289975}. %Given that our design philosophy is explicitly concerned with issues of cognitive cost, it is natural to consider these common cognitive hazards.
...
Another approach is to use a \emph{post hoc} static analysis that attempts to discover such problems \cite{spishak2012type}.
...
Problems like these arise whenever a programmer attempts to deploy dynamic string parsing as a solution to the problem of high syntactic cost. % The reason is that syntactic cost is a property of a drawing of a program, so trying to address it by drawing a different program requires establishing that the alternative program is equivalent to the program that the client would write if syntactic cost was not a consideration (which is, at worst, an ill-posed problem, and at best, a rather difficult problem.) %Moreover, logically equivalent programs can differ in terms of performance.
(There are, of course, valid applications of dynamic string parsing that are not motivated by the desire to decrease syntactic cost, e.g. when parsing string encodings of regular expressions received as dynamic input to the program.)%Strings are, simply put, not ideally suited for this task. 

\subsection{Dynamic Quotation Parsing}\label{sec:dynamic-quotation}
Some syntax dialects of ML, e.g. a syntax dialect that can be activated by toggling a compiler flag in the SML/NJ compiler \cite{SML/Quote}, define \emph{quotation literals}:  derived forms for expressions of type \li{'a frag list}, where \li{'a frag} is defined as follows:
\begin{lstlisting}[numbers=none]
...
\end{lstlisting}
It is possible to define a function \li{qparse : R.t frag list -> R.t} in \li{RXUtil} that parses fragment lists like these to produce regex values (or raises an exception if the fragment list cannot be parsed.)

Similarly, it is possible to define a function \li{qparse : 'a frag list -> 'a list} in the \li{List} module that can be applied to construct a list from a quoted list form:
\begin{lstlisting}[numbers=none]
...
As with dynamic string parsing, parsing occurs dynamically. We cannot use the trick of lifting all calls to \li{qparse} to the top level of our program, because the arguments are no longer string literals. At best, we can lift these calls into the earliest possible ``phase of evaluation''. Parse errors in quoted portions are only detected when this phase is entered, and the dynamic cost of parsing is incurred each time this phase is entered. For example, \li{List.qparse} is called $n$ times below, where $n$ is the length of \li{input}:
\begin{lstlisting}[numbers=none]
List.map (x => List.qparse `SCSS[^ECSSxSCSS, ^(ECSS2 * xSCSS)]ECSS`) input
\end{lstlisting}

  % the application of \li{qparse} is evaluated.
The programmer cannot specify alternative antiquotation delimiters -- the caret is the only choice in SML/NJ. This is problematic for regexes, for example, because the caret has a different meaning in the POSIX standard (and in practice, appears quite often.) 

Another problem is that all antiquoted values within a quotation literal must be of the same type. If, for example, we sought to support both spliced regexes and spliced strings in quoted regexes, we would need to define an auxiliary sum type in \li{RXUtil} 
and the client would need to wrap each antiquoted expression with a call to the corresponding constructor based on its type. 
For example, \li{lookup_rx} would be drawn as follows:
\begin{lstlisting}[numbers=none]
fun lookup_rx(string : name) =>
  RU.qparse' `SCSS^(ECSSRU.QS nameSCSS): ^(ECSSRU.QR readingSCSS)ECSS`
\end{lstlisting}
Similarly, if we sought to support quoted lists where the tail is explicitly given by the client (following OCaml's revised syntax \cite{ocaml-manual}), clients would need to apply marking constructors:
\begin{lstlisting}[numbers=none]
...
Finally, quotation parsing, like the other approaches considered so far, helps only with the problem of abbreviating expressions. It provides no solution to the problem of abbreviating patterns (because parse functions compute values, not patterns.)% The reason is simple: these approaches require applying functions, which, by nature, are expressions, not patterns.
...
infixr 5 ::
\end{lstlisting}
This directive causes expressions of the form \li{e1 :: e2} to desugar to \li{op:: e1 e2}, i.e. the variable \li{op::} is applied first to \li{e1}, then to \li{e2}. If \li{op::} is bound to the list constructor \li{Cons}, this expression constructs a list with head \li{e1} and tail \li{e2}.

The fixity directive above also causes patterns of the form \li{p1 :: p2} to desugar to \li{op:: p1 p2}, i.e. to pattern constructor application. Unfortunately, the datatype declaration that we gave in Sec. \ref{sec:lists} introduced a pattern constructor identified as \li{Cons}, not \li{op::}, so this infix form does not desugar to a valid list pattern. There is no mechanism for defining pattern constructor abbreviations in SML. The SML Basis library uses the label \li{op::} rather than \li{Cons} directly in its definition of the list datatype for this reason.
...
The simplest of these systems, e.g. Griffin's early work on notational definitions \cite{5134} and the syntax definition system integrated into the Agda programming language \cite{DBLP:conf/ifl/DanielssonN08}, support only forms that contain a fixed number of sub-trees, e.g. \li{if _ then _ else _}. SML-style derived list forms cannot be defined using these systems, because these forms contain a variable number of sub-expressions.
...
\end{lstlisting}
Here, the final declaration handles $n$-ary sequence of semi-colon separated trees.
...
These systems include various mechanisms based on generalized notions of precedence that help users manage this problem heuristically. For example, Agda supports a system of directed acyclic precedence graphs \cite{DBLP:conf/ifl/DanielssonN08} (which is related to earlier work by Aasa where a complete precedence graph was necessary \cite{DBLP:journals/tcs/Aasa95}.) In Coq, the programmer can associate notation definitions with ``scopes''. A scope can  be activated or deactivated explicitly using scope directives to control the availability of notation definitions. The innermost active scope has the highest precedence. In some situations, Coq is able to use type information to activate a scope implicitly.

\subsection{More General Syntax Definition Systems}\label{sec:syntax-dialects}
More general syntax definition systems give programmers more direct control over desugaring. We will first review these more general syntax definition systems, then give two examples that demonstrate the expressive power of these systems, followed by a discussion of the difficulties that programmers can expect to encounter if they  adopt these systems ``in the large'' (as a follow-up to what was discussed in Section \ref{sec:problems-with-dialects}.)
...
\emph{Parser combinator systems} define a functional interface for defining a parser, together with various functions that generate new parsers from existing parsers and other given values (these functions are referred to as the \emph{parser combinators}) \cite{Hutton1992d}. 
In some cases, it is acceptable to take the composition of various parser combinators as definitional (as opposed to the usual view, where a parser is an \emph{implementation} of a syntax definition.)
...
Syntax dialects implemented using parser combinators, or generated from a grammar by a {parser generator}, usually operate as language-external \emph{preprocessors}, transforming source text into a well-formed program. Some compilers, e.g. the OCaml compiler \cite{ocaml-manual}, integrate preprocessing into the build system. Other systems use directives placed in source text to invoke a preprocessor. For example, in Racket's reader macro system, the programmer can direct the lexer (called the ``reader'') to shift control to a given parser when a designated directive or token is seen \cite{Flatt:2012:CLR:2063176.2063195}.
...
\end{figure}
Syntax definition system like those just described called, we can define a dialect of VerseML's textual syntax called  $\mathcal{V}_\texttt{rx}$ that builds in derived forms based directly on the POSIX standard.\footnote{Technically, $\mathcal{V}_\texttt{rx}$ is a dialect of the textual syntax of VerseML's inner core language; see Chapter \ref{chap:uetsms}.} This dialect builds in derived forms related to the recursive type \li{rx} that was defined in Figure \ref{fig:datatype-rx}.

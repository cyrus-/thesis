...
\item The final premise of Rule (\ref{rule:expandsU-tsmap}) \emph{validates} the candidate expansion and simultaneously generates the \emph{final expansion}, $e$, according to the \emph{candidate expansion expression validation judgement}, described next.
\end{enumerate}
...
\begin{pfsteps*}
  ...
  \item $\hastypeU{\Delta}{\Gamma, \Ghyp{x}{\tau_1}}{e'}{\tau_2}$ \BY{IH on \pfref{expandsU}} \label{hastypeU}
...
\begin{pfsteps*}
  ...
  \item $\hastypeU{\Delta}{\Gamma}{e_1}{\aparr{\tau_1}{\tau}}$ \BY{IH on \pfref{expandsU1}}\pflabel{hastypeU1}
...
\item[\text{(\ref{rule:expandsU-tlam}) through (\ref{rule:expandsU-case})}] These cases follow analagously.
\\
...
\begin{pfsteps*}
  ...
  \item $\hastypeU{\Dcons{\Delta}{\Delta_S}}{\Gcons{\Gamma', \Ghyp{x}{\tau}}{\Gamma_S}}{x}{\tau}$ \BY{Lemma \ref{lemma:weakening-U} to \pfref{hastypeU} over $\Gamma_S$}
\end{pfsteps*}
\resetpfcounter
...
\begin{pfsteps*}
  ...
  \item $\hastypeU{\Dcons{\Delta}{\Delta_S}}{\Gcons{\Gamma, \Ghyp{x}{\tau_1}}{\Gamma_S}}{e'}{\tau_2}$ \BY{IH on \pfref{cvalidE}} \pflabel{hastype1}
...
\begin{pfsteps*}
  ...
  \item $\hastypeU{\Dcons{\Delta}{\Delta_S}}{\Gcons{\Gamma}{\Gamma_S}}{e_1}{\aparr{\tau_1}{\tau}}$ \BY{IH on \pfref{cvalidE1}} \pflabel{hastypeU1}
  \item $\hastypeU{\Dcons{\Delta}{\Delta_S}}{\Gcons{\Gamma}{\Gamma_S}}{e_2}{\tau_1}$ \BY{IH on \pfref{cvalidE2} \pflabel{hastypeU2}}
...
The mutual induction used to prove Theorems \ref{thm:typed-expansion-U} and \ref{thm:candidate-expansion-validation-U} is justified by showing that the following numeric metric on the subjects of the rule induction is decreasing:
\begin{align*}
...
This is established by the following two conditions. The first condition simply states that subsequences of $b$ are no longer than $b$.
\begin{condition}[Body Subsequences] If $\bsubseq{b}{m}{n}=b'$ then $\sizeof{b'} \leq \sizeof{b}$. \end{condition}
...
\begin{condition}[Body Parsing] If $\parseUExp{b}{\ue}$ then $\sizeof{\ue} < \sizeof{b}$.\end{condition}
...
Proving Theorem \ref{thm:candidate-expansion-validation-U} also requires the following lemma about candidate expansion type validation.
...
\begin{pfsteps*}
   ...
   \item $\istypeU{\Dcons{\Delta', \Dhyp{t}}{\Delta_S}}{t}$ \BY{iterated application of Lemma \ref{lemma:weakening-U} over $\Delta_S$ to \pfref{istype}.}
 \end{pfsteps*} 
\resetpfcounter
...
\begin{pfsteps*}
  ...
  \item $\istypeU{\Dcons{\Delta}{\Delta_S}, \Dhyp{t}}{\tau'}$ \BY{iterated exchange over $\Delta_S$ on \pfref{istypeU1}} \pflabel{istypeU2}
...
\item $\istypeU{\Dcons{\Delta}{\Delta_S}}{\tau}$ \BY{iterated application of Lemma \ref{lemma:weakening-U} and exchange over \pfref{istype}}
\end{pfsteps*}
\end{byCases}
\end{proof}

...
\paragraph{Shared Forms} Rules (\ref*{rule:expandsU-var}) through (\ref*{rule:expandsU-case}) define typed expansion of unexpanded expressions of shared form. These rules are each based on the corresponding typing rule, Rules (\ref{rule:hastypeU-var}) through (\ref{rule:hastypeU-case}), respectively. For example, the following typed expansion rules are based on the typing rules (\ref{rule:hastypeU-var}), (\ref{rule:hastypeU-lam}) and (\ref{rule:hastypeU-ap}), respectively:% for unexpanded expressions of variable, function and application form, respectively: 
...
\end{equation}
Observe that, for each of these rules, the unexpanded and expanded expression forms in the conclusion correspond, and the premises correspond to those of the corresponding typing rule. In particular, type formation premises correspond directly, and typed expansion premises correspond to the typing premises from the corresponding typing rule. The ueTSM environment, $\Sigma$, passes opaquely through these rules.
...
\end{split}\]
and so on for the remaining expanded expression forms.
\item When $e$ is of indefinite form, $\Uof{e}$ is a uniquely corresponding metatheoretic pattern also of indefinite form. For example, in Rule (\ref{rule:hastypeU-ap}), $e_1$ and $e_2$ are of indefinite form, i.e. they match arbitrary expanded expressions. The rule transformation simply ``hats'' them, i.e. $\Uof{e_1}=\ue_1$ and $\Uof{e_2}=\ue_2$.
\end{itemize}

It is instructive to use this rule transformation to generate Rules (\ref{rule:expandsU-var}) through (\ref{rule:expandsU-ap}) above. We omit the remaining rules, i.e. Rules (\ref*{rule:expandsU-tlam}) through (\ref*{rule:expandsU-case}). By instead defining these rules by the rule transformation just described, we avoid having to list a number of rules that are individually uninteresting. Moreover, this approach makes our exposition somewhat robust to changes to the inner core (though not to changes to the judgement forms in the statics of the inner core).% Even if changes to the judgement forms in the statics of the inner core are needed (e.g. the addition of a symbol context), it is easy to see would correspond to changes in the generic specification above.
% \begin{subequations}\label{rules:expandsU}
% \begin{equation}\label{rule:expandsU-var}
%   \inferrule{ }{\expandsU{\Delta}{\Gamma, x : \tau}{\Sigma}{x}{x}{\tau}}
% \end{equation}
...
\item The second premise types and expands the {unexpanded parse function} to generate the \emph{expanded parse function}, $\eparse$. Expanded parse functions are applied statically (i.e. during typed expansion) to encodings of literal bodies and either generate encodings of candidate expansions or indicate a parse error (as we will discuss when describing ueTSM application in the next subsection), so %Notice that this occurs under empty contexts, i.e. parse functions cannot refer to the surrounding bindings. 
...
\[\tParseResultExp \triangleq [\mapitem{\lbltxt{Success}}{\tCEExp}, \mapitem{\lbltxt{ParseError}}{\prodt{}}]\] and 
 $\tBody$ and $\tCEExp$ abbreviate types that classify encodings of literal bodies and candidate expansions. We will further characterize these types below.

 Evaluation is defined only for closed terms, so parse functions are not given access to surrounding bindings. 

\item ueTSM environments are finite mappings from TSM names, $a$, to \emph{expanded ueTSM definitions}, $\xuetsmdef{\tau}{\eparse}$, where $\tau$ is the ueTSM's {type annotation} and $\eparse$ is its {expanded parse function}. The \emph{ueTSM environment formation judgement}, $\uetsmenv{\Delta}{\Sigma}$, ensures that the type annotations in $\Sigma$ are well-formed assuming $\Delta$, and that the parse functions in $\Sigma$ are  of type $\aparr{\tBody}{\tParseResultExp}$.
\[\begin{array}{ll}
\textbf{Judgement Form} & \textbf{Description}\\
...
Again, defining the type $\tCEExp$ (and other associated types) explicitly, and these judgements inductively against these definitions, would be tedious, uninteresting, and overly specific. Instead, we state the following conditions, which ensure an isomorphism between values of type $\tCEExp$ and candidate expansion expressions.
...
\end{array}$
\caption[Syntax of candidate expansion types, expressions, rules and patterns in $\miniVersePat$]{Abstract syntax of candidate expansion types, expressions, rules and patterns in $\miniVerseUE$.}

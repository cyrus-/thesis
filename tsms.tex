...
The {inner core} forms a purely functional language with support for partial functions, quantification over types, recursive types, labeled product types and labeled sum types. We will tersely specify the statics of the inner core, and outline the structural dynamics, in the next two subsections. The reader is directed to \emph{PFPL} \cite{pfpl} (or another text on type systems, e.g. \emph{TAPL} \cite{tapl}) for a detailed introductory account of these constructs, or minor variants thereof.
...
\end{equation}
\end{subequations}
Rules (\ref{rules:istypeU}) and (\ref{rules:hastypeU}) are syntax-directed, so we assume an inversion lemma for each rule as needed without stating it separately or proving it explicitly. The following standard lemmas also hold. 

The Weakening Lemma establishes that extending a context with unused hypotheses preserves well-formedness and typing.
...
\begin{proof-sketch}
\begin{enumerate}
\item By rule induction on Rules (\ref{rules:istypeU}) and case analysis on the definition of substitution. In all cases, the derivation of $\istypeU{\Delta}{[\tau'/t]\tau}$ does not depend on the form of $\tau'$.
%\item Context formation of $[\tau'/t]\Gamma$ does not depend on the structure of $\tau'$.
\item By rule induction on Rules (\ref{rules:hastypeU}) and case analysis on the definition of substitution. In all cases, the derivation of $\hastypeU{\Delta}{[\tau'/t]\Gamma}{[\tau'/t]e}{[\tau'/t]\tau}$ does not depend on the form of $\tau'$.
\item By rule induction on Rules (\ref{rules:hastypeU}) and case analysis on the definition of substitution. In all cases, the derivation of $\hastypeU{\Delta}{\Gamma}{[e'/x]e}{\tau}$ does not depend on the form of $e'$.
\end{enumerate}
\end{proof-sketch}

The Regularity Lemma establishes that the type assigned to an expression under a well-formed typing context is well-formed. 
...
The Preservation and Progress conditions establish that evaluation of a well-typed expression preserves typing and cannot ``get stuck''. Together, these two conditions constitute the Type Safety condition.
...
\[\begin{array}{ll}
\textbf{Judgement Form} & \textbf{Description}\\
\expandsUX{\ue}{e}{\tau} & \text{$\ue$ expands to $e$ at type $\tau$ under ueTSM environment $\Sigma$}\\
...
\noindent
The typed expansion judgement is inductively defined by Rules (\ref*{rules:expandsU}), which are defined as follows.

\paragraph{Shared Forms} Typed expansion of unexpanded expressions that are of a shared form is straightforward. Each typing rule, i.e. Rules (\ref{rule:hastypeU-var}) through (\ref{rule:hastypeU-case}) above, corresponds to a typed expansion rule, Rules (\ref*{rule:expandsU-var}) through (\ref*{rule:expandsU-case}), respectively. For example, the typing rules (\ref{rule:hastypeU-var}), (\ref{rule:hastypeU-lam}) and (\ref{rule:hastypeU-ap}) correspond to the following typed expansion rules:% for unexpanded expressions of variable, function and application form, respectively: 
...
\end{equation}
In each of these typed expansion rules, the unexpanded and expanded expression forms in the conclusion correspond, and the premises correspond to those of the corresponding typing rule. In particular, type formation premises correspond directly, and typed expansion premises correspond to the typing premises from the corresponding typing rule. The ueTSM environment, $\Sigma$, need not yet be discussed because it passes opaquely through these rules.

We can express this intuition more precisely with the following rule transformation. For each rule in Rules (\ref{rules:hastypeU}),
\begin{mathpar}
...
\end{split}\]
and where $\Uof{e}$, when $e$ is a metatheoretic pattern of sort $\mathsf{EExp}$, is a metatheoretic pattern of sort $\mathsf{UExp}$ defined as follows.
\begin{itemize}
\item When $e$ is of definite form,
\[\begin{split}
\Uof{x} & = x\\
\Uof{\aelam{\tau}{x}{e}} & = \aulam{\tau}{x}{\Uof{e}}\\
\Uof{\aeap{e_1}{e_2}} & = \auap{\Uof{e_1}}{\Uof{e_2}}
\end{split}\]
and so on for the remaining expanded expression forms.
\item When $e$ is of indefinite form, $\Uof{e}$ is a uniquely corresponding metatheoretic pattern also of indefinite form. For example, in Rule (\ref{rule:hastypeU-ap}), $e_1$ and $e_2$ are of indefinite form, i.e. they match arbitrary expanded expressions. The rule transformation simply ``hats'' $e_1$ and $e_2$, i.e. $\Uof{e_1}=\ue_1$ and $\Uof{e_2}=\ue_2$.
\end{itemize}

It is instructive to use this rule transformation to generate Rules (\ref{rule:expandsU-var}) through (\ref{rule:expandsU-ap}) above. We omit the remaining rules, i.e. Rules (\ref*{rule:expandsU-tlam}) through (\ref*{rule:expandsU-case}). By instead defining these rules by the rule transformation just described, we do not need to list a number of rules that are individually uninteresting. Moreover, this approach makes our exposition more robust to changes to the inner core (as long as these changes do not require changes to the judgement forms in the statics of the inner core).% Even if changes to the judgement forms in the statics of the inner core are needed (e.g. the addition of a symbol context), it is easy to see would correspond to changes in the generic specification above.
% \begin{subequations}\label{rules:expandsU}
% \begin{equation}\label{rule:expandsU-var}
%   \inferrule{ }{\expandsU{\Delta}{\Gamma, x : \tau}{\Sigma}{x}{x}{\tau}}
% \end{equation}
...
% \end{equation}
\end{subequations}
\paragraph{ueTSM Definition and Application} The rules governing the two ``interesting'' forms, Rules (\ref{rule:expandsU-syntax}) and (\ref{rule:expandsU-tsmap}), are defined in the next two subsections, respectively. 
...
ueTSM environments are finite functions from TSM names, $a$, to \emph{expanded ueTSM definitions}, $\xuetsmdef{\tau}{\eparse}$, where $\tau$ is the ueTSM's {type annotation} and $\eparse$ is its {expanded parse function}. The \emph{ueTSM environment formation judgement}, $\uetsmenv{\Delta}{\Sigma}$, ensures that the type annotations in $\Sigma$ are well-formed assuming $\Delta$, and that the parse functions in $\Sigma$ are  of type $\aparr{\tBody}{\tParseResultExp}$.
\[\begin{array}{ll}
\textbf{Judgement Form} & \textbf{Description}\\
...
\end{subequations}
\item 
The fourth premise of Rule (\ref{rule:expandsU-syntax}) extends the ueTSM environment with the newly determined expanded ueTSM definition and proceeds to assign a type, $\tau'$, and expansion, $e$, to $\ue$.
\end{enumerate}
The conclusion of Rule (\ref{rule:expandsU-syntax}) assigns the type $\tau'$ and expansion $e$ to the definition as a whole, i.e. TSM definitions ``disappear'' when an unexpanded expression is expanded (because they specify behavior that is relevant only during typed expansion). 

\subsection{ueTSM Application}\label{sec:U-uetsm-application}
The last unexpanded expression form is the form for applying the ueTSM named $a$ to a literal form with literal body $b$:
\[
\utsmap{a}{b}
\] 
The stylized form for ueTSM application, above, uses forward slashes to delimit the literal body, though stylized variants of any of the literal forms specified in Figure \ref{fig:literal-forms} would be straightforward to add to Figure \ref{fig:U-unexpanded-terms} (we omit them for simplicity).

The corresponding operational form is $\autsmap{b}{a}$, i.e. for each literal body $b$, the operator $\texttt{uapuetsm}[b]$ is indexed by the TSM name $a$ and takes no arguments. %\footnote{This is in following the conventions in \emph{PFPL} \cite{pfpl}, where operators parameters allow for the use of metatheoretic objects that are not syntax trees or binding trees, e.g. $\mathsf{str}[s]$ and $\mathsf{num}[n]$.} This operator is indexed by the TSM name $a$ and takes no arguments. 
TSM names are symbolic, i.e. they can be compared for equality and disequality, but unlike variables, they do not stand for terms. Instead, they operate as references into the ueTSM environment. 

The typed expansion rule governing unexpanded expressions of this form is below:
\begin{subequations}[resume]
\begin{equation}\label{rule:expandsU-tsmap}
\inferrule{
  \encodeBody{b}{\ebody}\\
...
\end{array}$
\caption[Syntax of candidate expansion types and expressions in $\miniVerseUE$]{Abstract syntax of candidate expansion types, $\ctau$ (pro\-nounced ``grave $\tau$''), and expressions, $\ce$, in $\miniVerseUE$. Metavariables $m$ and $n$ range over natural numbers.}
...
The \emph{candidate expansion expression validation judgement}, $\cvalidE{\Delta}{\Gamma}{\escenev}{\ce}{e}{\tau}$, is defined mutually inductively with the typed expansion judgement as follows.% This is necessary because a typed expansion judgement appears as a premise in Rule (\ref{rule:cvalidE-U-splicede}) below, and a candidate expansion expression validation judgement appears as a premise in Rule (\ref{rule:expandsU-tsmap}) above.

\paragraph{Corresponding Forms}
...
\end{equation}
Formally, Rules (\ref*{rule:cvalidE-U-var}) through (\ref*{rule:cvalidE-U-case}) are defined as follows. 
%Rather than defining Rules (\ref*{rule:cvalidE-U-tlam}) through (\ref*{rule:cvalidE-U-case}) explicitly, we define them (as well as Rules (\ref*{rule:cvalidE-U-var}) through (\ref*{rule:cvalidE-U-ap}) above) as follows. 
For each rule in Rules (\ref{rules:hastypeU}),
...
\end{mathpar}
we define the corresponding candidate expansion expression validation rule
\begin{mathpar}
  \inferrule{
    \VExpof{J_1}\\
    \cdots\\
...
Notice that in Rule (\ref{rule:cvalidE-U-var}), only variables tracked by the candidate expansion's typing context, $\Gamma$, are validated. Variables assigned a type in the application site typing context, which is in the expression splicing scene $\escenev$, are not validated. Indeed, $\escenev$ is not inspected by any of the rules above. This achieves \emph{context-independent expansion} as described in Sec. \ref{sec:splicing-and-hygiene} -- ueTSMs cannot impose ``hidden constraints'' on the application site typing context, because the variable bindings at the application site are not available to candidate expansions directly.

\paragraph{Spliced Unexpanded Expressions}

The only candidate expansion expression form that does not correspond to an expanded expression form is $\acesplicede{m}{n}$. Candidate expansion expressions of this form are \emph{references to a spliced unexpanded expression}, i.e. they indicate that an unexpanded expression should be parsed out from the literal body, $b$, beginning at position $m$ and ending at position $n$. Rule (\ref{rule:cvalidE-U-splicede}) governs this form. 
...
\end{equation}  
\end{subequations}
The first premise of Rule (\ref{rule:hastypeUP-match}) assigns a type, $\tau$, to the scrutinee, $e$. The second premise checks that the type of the expression as a whole, $\tau'$, is well-formed.\footnote{This premise, and the type argument in the match form, are necessary to maintain regularity, defined below, but only because when $n=0$ the type $\tau'$ is arbitrary.} The third premise then ensures that each of the $n \geq 0$ rules $r_i$, for $1 \leq i \leq n$, take values of type $\tau$ to values of the type of the match expression as a whole, $\tau'$. This is expressed by the \emph{rule typing judgement}, $\ruleType{\Delta}{\Gamma}{r}{\tau}{\tau'}$, which is defined mutually with Rules (\ref{rules:hastypeUP}) by the following rule:
...
As in Sec. \ref{sec:dynamics-U}, our subsequent developments do not make mention of particular rules in the dynamics, nor the auxiliary judgements for specifying the dynamics of the match operator, not shown, so we do not provide these details here. Instead, it suffices to state the following conditions.
...
The Preservation and Progress conditions (together, the {Type Safety} condition) ensure that the statics of the inner core and the structural dynamics are coherent, i.e. that evaluation of a well-typed expression preserves typing and cannot ``get stuck''.
...
The surface syntax of $\miniVersePat$ consists of types, as above, unexpanded expressions, $\ue$, unexpanded rules, $\urv$, and unexpanded patterns, $\upv$. The syntax of unexpanded terms is specified in Figure \ref{fig:UP-unexpanded-terms}. 
...
&&& \acepr{\ell}{\ce} & \prj{\ce}{\ell} & \text{projection}\\
&&& \acein{\labelset}{\ell}{\mapschema{\ctau}{i}{\labelset}}{\ce} & \inj{\ell}{\ce} & \text{injection}\\
&&& \acematchwith{n}{\ce}{\tau}{\seqschemaX{\crv}} & \matchwith{\ce}{\seqschemaX{\crv}} & \text{match}\\
&&& \acesplicede{m}{n} & \splicede{m}{n} & \text{spliced}\\

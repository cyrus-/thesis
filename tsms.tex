...
\end{array}$
\caption[Syntax of types and expanded expressions in $\miniVerseU$.]{Syntax of types and expanded expressions, which form the inner core of $\miniVerseU$. Metavariable $x$ ranges over variables, $t$ ranges over type variables, $\ell$ ranges over labels and $\labelset$ ranges over label sets. We write $\mapschema{\tau}{i}{\labelset}$ for a sequence of operator arguments $\tau_i$, one for each $i\in \labelset$ (and similarly for other sorts). Terms indexed by label sets are identified up to mutual reordering of the label set and the argument sequences indexed by that label set. In the stylized syntax, the label set is omitted when it can be inferred, e.g. the labeled product type $\prodt{\finmap{\mapitem{\ell_1}{e_1}, \mapitem{\ell_2}{e_2}}}$ is implicitly indexed by label set $\{\ell_1, \ell_2\}$. When we use the stylized forms $\fold{e}$ and $\inj{\ell}{e}$, we assume that the reader can infer the type information made explicit only in the corresponding abstract form from the surrounding context.}
...
\item If $\tau=\aprod{\labelset}{\mapschema{\tau}{i}{\labelset}}$ then $e=\aetpl{\labelset}{\mapschema{e}{i}{\labelset}}$ and for each $i \in \labelset$ we have that $\hastypeUC{e_i}{\tau_i}$ and $\isvalU{e_i}$.
...
\begin{equation}\label{rule:cvalidT-U-prod}
  \inferrule{
    ...
    \cvalidT{\Delta}{\spctxv}{\aceprod{\labelset}{\mapschema{\ctau}{i}{\labelset}}}{\aprod{\labelset}{\mapschema{\tau}{i}{\labelset}}}
  }
\end{equation}
\begin{equation}\label{rule:cvalidT-U-sum}
  \inferrule{
    ...
    \cvalidT{\Delta}{\spctxv}{\acesum{\labelset}{\mapschema{\ctau}{i}{\labelset}}}{\asum{\labelset}{\mapschema{\tau}{i}{\labelset}}}
  }
\end{equation}
...
\item By mutual rule induction on Rules (\ref{rules:hastypeUP}) and Rule (\ref{rule:ruleType}).
\item By mutual rule induction on Rule (\ref{rule:ruleType}) and Rules (\ref{rules:hastypeUP}).
\item By mutual rule induction on Rule (\ref{rule:ruleType}) and Rules (\ref{rules:hastypeUP}).
\end{enumerate}
\end{proof-sketch}
...
\item The derivation of $\hastypeU{\Delta}{[\tau'/t]\Gamma}{[\tau'/t]e}{[\tau'/t]\tau}$ does not depend on the structure of $\tau'$.
\item Typing of $[e'/x]e$ depends only on the type of $e'$ wherever it occurs, if at all.
\item The derivation of $\ruleType{\Delta}{[\tau''/t]\Gamma}{[\tau''/t]r}{[\tau''/t]\tau}{[\tau''/t]\tau'}$ does not depend on the structure of $\tau'$.
\item Typing of $[e/x]r$ depends only on the type of $e$ wherever it occurs, if at all.
\end{enumerate}
\end{proof-sketch}
...
\begin{enumerate}
\item By mutual rule induction on Rules (\ref{rules:hastypeUP}) and Rule (\ref{rule:ruleType}).
 and Lemma \ref{lemma:substitution-UP}.
\item By mutual rule induction on Rule (\ref{rule:ruleType}) and Rules (\ref{rules:hastypeUP}).
\end{enumerate}
\end{proof-sketch}
...
\item If $\tau=\arec{t}{\tau'}$ then $e=\aefold{t}{\tau'}{e'}$ and $\hastypeUC{e'}{[\abop{rec}{t.\tau'}/t]\tau'}$. 
\item If $\tau=\aprod{\labelset}{\mapschema{\tau}{i}{\labelset}}$ then $e=\aetpl{\labelset}{\mapschema{e}{i}{\labelset}}$ and for each $i \in \labelset$ we have that $\hastypeUC{e_i}{\tau_i}$ and $\isvalU{e_i}$.
\item If $\tau=\asum{\labelset}{\mapschema{\tau}{i}{\labelset}}$ then $e=\aein{\labelset}{\ell}{\mapschema{\tau}{i}{\labelset\setminus \ell} \otimes \ell \hookrightarrow \tau'}{e'}$ and $\ell \in \labelset$ and $\hastypeUC{e'}{\tau'}$ and $\isvalU{e'}$.
\end{enumerate}\end{condition}

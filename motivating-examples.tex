...
\end{lstlisting}
Here, \li{list} is a {type function} binding its type parameter to the type variable \li{'a}. We apply parameters in post-fix position (rather than in prefix position, as in SML.) For example, the type of integer lists is \li{list(int)}. This is equivalent, by substitution of \li{int} for \li{'a}, to the following \emph{recursive type}:
\begin{lstlisting}[numbers=none]
...
\end{lstlisting}
The values of a labeled sum type \li{T} are injections \li{inj[Lbl](e)}, where \li{Lbl} is a label specified by one of the variants of \li{T} and \li{e} is a value of the corresponding type. the {labeled sum type} above specifies two \emph{variants}:
\begin{enumerate}
\item One variant, labeled \li{Nil}, takes values of \li{unit} type (we can omit \li{of unit}.) The only value of \li{unit} type is the trivial value \li{()}.  
\item The other variant, labeled \li{Cons}, takes values of the \emph{product type} \li{int * list(int)}, the values of which are tuples. 
\end{enumerate}

Taken together, we can define two example values of type \li{list(int)} as follows:
\begin{lstlisting}[numbers=none]

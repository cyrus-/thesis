...
In Standard ML, list types arise out of the following parameterized recursive datatype:
\begin{lstlisting}[numbers=none]
datatype 'a list = nil | op:: of 'a * 'a list
\end{lstlisting}
This declaration generates:
\begin{itemize}
\item a type function \li{list} that binds one type parameter to the type variable \li{'a}; 
...
With that in mind, let us examine how the family of list types is defined in VerseML:
\begin{lstlisting}[numbers=none]
type 'a list = rec(self => Nil + Cons of 'a * self)
\end{lstlisting}
Here, \li{list} is a {type function} binding its type parameter to the type variable \li{'a}. We apply parameters in prefix position, as in SML. For example, the type of integer lists is \li{int list}, which is equivalent, by substitution of \li{int} for \li{'a}, to the following \emph{recursive type}:
\begin{lstlisting}[numbers=none]
rec(self => Nil + Cons of int * self)
\end{lstlisting}
%Here, the type variable \li{self} is bound as a \emph{self reference} in the type body. 
The values of a recursive type \li{T} are \li{(fold e) : T}, where \li{e} is a value of the \emph{unrolling} of \li{T}. The {unrolling} of a recursive type is determined by substituting the recursive type itself for the self reference, here \li{self}, in its type body. For example, the unrolling of \li{int list} is equivalent, by substitution of \li{int list} for \li{self}, to the following:
\begin{lstlisting}[numbers=none]
Nil + Cons of int * int list
\end{lstlisting}
This \emph{labeled sum type} specifies two \emph{variants}. One, labeled \li{Nil}, takes values of \li{unit} type (we could have written \li{Nil of unit}.) The only value of \li{unit} type is the trivial value \li{()}.  The other variant, labeled \li{Cons}, takes values of the \emph{product type} \li{int * int list}, the values of which are tuples. The values of a labeled sum type \li{T} are injections \li{(inj[Lbl] e) : T}, where \li{Lbl} is a label specified by one of the variants of \li{T} and \li{e} is a value of the corresponding type. 

Taken together, we can define two example values of type \li{int list} as follows:
\begin{lstlisting}[numbers=none]
val nil_int : int list = fold(inj[Nil] ())
val one_int : int list = fold(inj[Cons] (1, nil_int))
\end{lstlisting}
...
One way to lower syntactic cost is to define the following polymorphic values, called the \emph{list value constructors}, which abstract away the folds and injections:
\begin{lstlisting}[numbers=none]

...
\begin{quote}
\emph{...a greater level of precision and technical
complexity in logical investigations arises because [formal languages] are precisely defined
...
\end{lstlisting}
The values of a labeled sum type \li{T} are injections \li{inj[Lbl](e)}, where \li{Lbl} is a label specified by one of the \emph{variants} specified by \li{T} and \li{e} is a value of the corresponding type. The {labeled sum type} above specifies two {variants}:
\begin{enumerate}
